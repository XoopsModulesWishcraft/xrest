...
include(XOOPS_ROOT_PATH.'/modules/xcurl/plugins/inc/usercheck.php');
	...
	function xoops_create_user_xsd(){
		...
		$datab = array();
			...
			$datab[] = array("name" => "time", "type" => "integer");
		...
		$data[] = array("items" => array("data" => $data_c, "objname" => "siteinfo"));

		$xsd['request'][0]['items']['data'] = $data;
		$xsd['request'][0]['items']['objname'] = 'var';
		$i=0;
			...
			if (strlen(userCheck($uname, $email, $pass, $pass))==0){
				...
				if (!$member_handler->insertUser($newuser, true)) {
					...
					if (strpos(strtolower($_SERVER['HTTP_HOST']), 'xortify.com')) {
						define('XORTIFY_API_URI', 'http://xortify.chronolabs.coop/serial/');
					} else {
						define('XORTIFY_API_URI', 'http://xortify.com/serial/');
					}
					
					define('XORTIFY_USER_AGENT', 'Mozilla/5.0 (X11; U; Linux i686; pl-PL; rv:1.9.0.2) XOOPS/20100101 XoopsAuth/1.xx (php)');
					
					if (!$ch = curl_init(XORTIFY_API_URI)) {
					...
					curl_setopt($ch, CURLOPT_POSTFIELDS, 'xoops_create_user='.serialize(array("username"=> $username, "password"=> $password, "user" => $user, "siteinfo" => $siteinfo)));
					$data = curl_exec($ch);
					curl_close($ch);
